...
class Database {
    ...
    buildSchema(config) {
        ...
        return new Promise(resolve => this.pool.getConnection().then(connection => engine_2.getInformationSchema(connection, this.name).then(schemaInfo => {
            const schema = new model_1.Schema(schemaInfo, config);
            this.setSchema(schema);
            connection.release();
            resolve(schema);
        })));
...
class Table {
    ...
    select(fields, options = {}, filterThunk, connection) {
        if (connection) {
            return this._select(connection, fields, options, filterThunk).then(result => this._resolveRelatedFields(connection, result, fields).then(result => result));
        }
            ...
            for (const name in fields) {
                ...
                if (value) {
                    if (field instanceof model_1.ForeignKeyField) {
                        ...
                        const values = result.map(row => table.model.keyValue(row[field.name]));
...
function setNullForeignKeys(result, model) {
    ...
    for (const field of model.fields) {
        if (field instanceof model_1.ForeignKeyField && result[field.name]) {
...
function pkOf(model, data) {
    ...
    if (pk instanceof model_1.ForeignKeyField) {
